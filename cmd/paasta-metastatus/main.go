...
func (o *PaastaMetastatusOptions) Setup() {
	...
	flag.StringVar(
		...
		"mode",
		"api",
		"api: Use PaaSTA API to invoke metastatus on master, direct: run metastatus logic directly",
	)
}
...
func main() {
	...
	var success bool
	if options.Mode == "api" {
		success, err = (&APIMode{}).metastatus(options)
	} else {
		success, err = (&DirectMode{}).metastatus(options)
	}
